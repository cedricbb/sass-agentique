...
function makeRequest() {
  return new NextRequest("http://localhost/api/account/quotes/test-id/file");
}
...
describe("GET /api/account/quotes/[id]/file", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns_200_with_pdf_stream_and_correct_headers", async () => {
    mockGetQuoteById.mockResolvedValue(makeQuote());
    ...
    expect(response.headers.get("Content-Disposition")).toBe(
      'inline; filename="devis-D-001.pdf"',
    );
  ...
  it("returns_404_when_quote_is_draft", async () => {
    mockGetQuoteById.mockResolvedValue(makeQuote({ status: "draft" }));
  ...
  it("returns_404_when_client_does_not_own_quote", async () => {
    mockGetQuoteById.mockResolvedValue(makeQuote({ clientId: "client-OTHER" }));
  ...
  it("returns_404_when_pdfKey_is_null", async () => {
    mockGetQuoteById.mockResolvedValue(makeQuote({ pdfKey: null }));
  ...
  it("returns_500_and_logs_on_generic_r2_error", async () => {
    ...
    expect(consoleSpy).toHaveBeenCalledWith(
      "[GET /api/account/quotes/[id]/file] R2 stream error",
      genericError,
    );
