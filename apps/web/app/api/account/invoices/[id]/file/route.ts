...
export async function GET(
  ...
  if (invoice.clientId !== scope.client.id) {
    return new Response("Not Found", { status: 404 });
  }
  ...
  try {
    const { body, contentLength } = await streamPdfFromR2(invoice.pdfKey);
    ...
    console.error("[GET /api/account/invoices/[id]/file] R2 stream error", err);
