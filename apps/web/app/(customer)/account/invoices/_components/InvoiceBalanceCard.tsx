    ...
    <Card className="max-w-xs" data-testid="invoice-balance-card">
      ...
      <CardContent className="space-y-2">
        <div className="flex justify-between">
          ...
          <span data-testid="invoice-balance-total-ttc">{formatCurrency(totalTtcCents / 100)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-sm text-muted-foreground">Déjà payé</span>
          <span data-testid="invoice-balance-paid">{formatCurrency(paidCents / 100)}</span>
        </div>
