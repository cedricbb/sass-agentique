...
- **Devis** : Server Actions (`actions/quotes.ts`, `actions/quote-items.ts`), pages liste et détail, composants `QuotesTable`, `QuoteForm`, `QuoteItemsEditor`, `EditQuoteItemDialog`, `QuoteStatusActions`, `QuoteToInvoiceButton`. `QuoteForm` expose un Select "Destinataire (contact)" filtré sur le client sélectionné (masqué tant qu'aucun client n'est choisi, reset auto au changement de client) ; en édition, pré-rempli avec `quote.contactId`. Voir `docs/admin-quotes.md`.
- **Factures** : Server Actions (`actions/invoices.ts`, `actions/invoice-items.ts`), pages liste et détail, composants `InvoicesTable`, `InvoiceForm`, `InvoiceItemsEditor`, `EditInvoiceItemDialog`, `InvoiceStatusActions`, `InvoiceAmountsCard`, `InvoiceBalanceCard`, `InvoicePaymentsList`, `RecordPaymentDialog`. `InvoiceForm` expose un Select "Destinataire (contact)" filtré sur le client sélectionné (reset auto au changement de client, masqué en mode from-quote) ; en édition, pré-rempli avec `invoice.contactId`. Service `listClientContactsByOwner(ownerId)` dans `client.service.ts` charge les contacts en une requête JOIN (évite le N+1 sur la page de création). Voir `docs/admin-invoices.md`.
